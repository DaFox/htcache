...
async fn main() {
	...
	either!(options.get_flag("ecs-logging"), ecs_logger::init(), pretty_env_logger::init());
	
	let address = options.get_one::<IpAddr>("addr").unwrap();
	let port    = options.get_one::<u16>("port").unwrap();

    let server = warp::serve(filters::cache_api(cache.clone()))
        .run(SocketAddr::new(*address, *port));
		
	futures::join!(
		cache_gc(60, cache.clone()),
		server
	);
}
...
mod service {
	...
	impl CacheRecord {
		fn is_expired(&self) -> bool {
			self.expires.map_or(false, |ttl| (self.created + Duration::seconds(ttl as i64)) < Utc::now())
		}
	...
	impl CacheService {
		...
		pub fn set(&mut self, key: &str, val: &str, ttl: Option<u32>, content_type: Option<String>) {
			self.storage.insert(Self::hash(key), CacheRecord {
				created: Utc::now(),
				expires: ttl,
...
mod filters {
	...
	pub fn cache_api(cache: CacheServiceTS) -> impl Filter<Extract = (impl warp::Reply,), Error = warp::Rejection> + Clone	{
		    cache_get(cache.clone())
		.or(cache_put(cache.clone()))
		.with(warp::log("api"))
	}
	
	pub fn cache_get(cache: CacheServiceTS) -> impl Filter<Extract = (impl warp::Reply,), Error = warp::Rejection> + Clone {
		warp::path!(String)
		  .and(warp::get())
	...
	pub fn cache_put(cache: CacheServiceTS) -> impl Filter<Extract = (impl warp::Reply,), Error = warp::Rejection> + Clone {
		warp::path!(String)
		  .and(warp::put())
		  .and(warp::body::content_length_limit(1024 * 128))
		  .and(warp::body::bytes().map(|bytes:Bytes| String::from_utf8(bytes.to_vec()).expect("error converting bytes to &str")))
...
mod handlers {
	...
	pub async fn cache_get(name: String, cache: CacheServiceTS) -> Result<impl warp::Reply, Infallible> {
		if let Some(record) = cache.lock().await.get(name.as_str()) {
			if let Some(content) = record.get() {
				return Ok(warp::http::Response::builder()
					.status(200)
					.header("Content-Type", record.get_content_type().unwrap_or(&"text/plain".to_string()))
	...
	pub async fn cache_put(name: String, body: String, content_type: Option<String>, ttl: Option<u32>, cache: CacheServiceTS) -> Result<impl warp::Reply, Infallible> {
		cache.lock().await.set(name.as_str(), &body, ttl, content_type);
